...
export default function DiceArea({ dice, onRoll, allocation, onAllocationChange }: Props) {
  ...
  return (
    <div className="flex flex-col gap-3 shrink-0 w-44">
      ...
      {/* Combine / Split */}
      <div className="flex flex-col gap-2 border-2 border-gray-600 rounded-2xl p-3 text-sm text-gray-300">
        <label className={`flex items-center gap-2 ${hasRolled ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'}`}>
          <input
            ...
            className="hidden"
          />
          <span className={`w-3.5 h-3.5 rounded-full border-2 shrink-0 inline-block transition-colors ${allocation === 'combine' ? 'border-blue-400 bg-blue-400' : 'border-gray-400'}`} />
          <span>Combine</span>
          <span className="ml-auto w-8 h-7 rounded border border-gray-500 bg-gray-800 flex items-center justify-center text-white font-bold">
        ...
        <label className={`flex items-center gap-2 ${hasRolled ? 'cursor-pointer' : 'cursor-not-allowed opacity-50'}`}>
          <input
            ...
            className="hidden"
          />
          <span className={`w-3.5 h-3.5 rounded-full border-2 shrink-0 inline-block transition-colors ${allocation === 'split' ? 'border-blue-400 bg-blue-400' : 'border-gray-400'}`} />
          <span>Split</span>
        ...
        <button
          ...
          className="mt-1 w-full py-2 rounded-xl bg-green-700 text-white font-bold text-sm transition-opacity disabled:opacity-40 disabled:cursor-not-allowed"
        >
          Submit
        </button>
      </div>
